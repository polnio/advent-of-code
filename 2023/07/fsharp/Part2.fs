...
module Part2 =
    ...
    let getCombination (cards: string) =
        if cards |> Seq.forall ((=) 'J') then
            Combination.Five
        else
            let (symbol, count_max) =
                cards
                |> Seq.countBy id
                |> Seq.filter (fun (symbol, _) -> symbol <> 'J')
                |> Seq.maxBy (fun (symbol, count) -> (count, cardSymbols |> Seq.findIndex ((=) symbol) |> (*) -1))

            let count = count_max + Seq.length (cards |> Seq.filter ((=) 'J'))
            ...
            | 3 ->
                if
                    cards
                    |> Seq.countBy id
                    |> Seq.exists (fun (s, count) -> count = 2 && s <> symbol && s <> 'J')
                then
